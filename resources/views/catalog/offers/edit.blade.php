...
@section('catalogTitle', ($pageTitle ?? 'Editar oferta').' · Catálogo')

@section('catalog')
    <div class="mb-4">
        <h1 class="text-xl font-bold text-gray-900">{{ $pageTitle ?? 'Editar oferta' }}</h1>
    ...
    <form action="{{ route('catalog.offers.update', $offer) }}" method="POST" enctype="multipart/form-data" class="bf-form-panel space-y-4">
        ...
        @include('catalog.offers._form', [
            'offer' => $offer,
            'products' => $products,
            'defaultType' => $defaultType ?? $offer->type,
            'lockType' => true,
        ])
