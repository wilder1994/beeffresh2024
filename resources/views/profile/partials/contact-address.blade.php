    ...
    <div class="space-y-3">
        @if($user->isSupplier())
            <div>
                ...
                <x-text-input id="company_name" name="company_name" type="text" :value="old('company_name', $user->company_name)" autocomplete="organization" />
        ...
        @if($user->isCustomer())
            <div>
                ...
                <x-text-input id="address_line1" name="address_line1" type="text" :value="old('address_line1', $user->address_line1)" autocomplete="street-address" />
            ...
            <div>
                ...
                <x-text-input id="address_line2" name="address_line2" type="text" :value="old('address_line2', $user->address_line2)" />
                <x-input-error class="mt-2" :messages="$errors->get('address_line2')" />
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                <div>
                    <x-input-label for="city" value="Ciudad / municipio" />
                    <x-text-input id="city" name="city" type="text" :value="old('city', $user->city)" autocomplete="address-level2" />
                    <x-input-error class="mt-2" :messages="$errors->get('city')" />
                </div>
                <div>
                    <x-input-label for="state" value="Provincia / estado" />
                    <x-text-input id="state" name="state" type="text" :value="old('state', $user->state)" autocomplete="address-level1" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                <div>
                    ...
                    <x-text-input id="postal_code" name="postal_code" type="text" :value="old('postal_code', $user->postal_code)" autocomplete="postal-code" />
                    <x-input-error class="mt-2" :messages="$errors->get('postal_code')" />
                </div>
                <div>
                    <x-input-label for="country" value="País (ISO 2 letras)" />
                    <x-text-input id="country" name="country" type="text" maxlength="2" :value="old('country', $user->country ?? 'DO')" autocomplete="country" />
                    <x-input-error class="mt-2" :messages="$errors->get('country')" />
                </div>
            </div>
            <div>
                <x-input-label for="delivery_instructions" value="Indicaciones para el domiciliario" />
                <textarea id="delivery_instructions" name="delivery_instructions" rows="3" class="bf-textarea min-h-[4rem]">{{ old('delivery_instructions', $user->delivery_instructions) }}</textarea>
