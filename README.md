...
**Tras login:** clientes → **`/`**; proveedores → `/portal-proveedor`; **domiciliarios** (`module.courier`) → `/domiciliario/pedidos`; **administradores** → `/admin/dashboard` (panel ejecutivo con ingresos); **despachadores** (`module.orders` + cargo `despachador`) → `/despacho/dashboard` (panel personal sin $); otros empleados con `module.orders` → `/admin/pedidos`; resto del personal → `/dashboard` (`App\Support\PostLoginRedirect`, `DashboardController`).
...
4. Assets (Vite incluye entradas de operaciones: `operationsPolling.js`, `operationsMap.js`, `courierOps.js`, `orderTracking.js`, `notificationBell.js`, `executiveDashboard.js`, `dispatcherDashboard.js`). **Tras cambiar JS/CSS del panel domiciliario, dashboards o realtime, recompila** — `public/build` está en `.gitignore` y el navegador no verá el pool en vivo ni otros parches DOM si el bundle local está desactualizado:
...
- **Flujo recomendado (local + ngrok):** trabaja en **`APP_LOCAL_URL`** (p. ej. `http://localhost:8080`). Ngrok solo entra en juego al pagar:
  ...
  3. **Al terminar el pago** → pantalla de éxito en el túnel (sin salto automático). El cliente elige **Ver seguimiento** o **Volver al catálogo** → enlace a **`APP_LOCAL_URL`** con *handoff* de sesión (`bf_local_handoff`).
  4. Si abres el túnel por error en catálogo o seguimiento → `RedirectTunnelBrowsingToLocalUrl` te devuelve a localhost (salvo rutas de pago: checkout, widget, retorno, poll `/pago/estado`, éxito/pendiente/fallo).
- El **carrito autenticado** se replica en caché (`CartStorage`, clave `cart.user.{id}`) entre localhost y ngrok. Al **aprobar el pago**, el webhook y el poll vacían esa caché por `user_id` (`forgetForUser`); la sesión localhost obsoleta no puede restaurarlo. El handoff `bf_local_handoff` sincroniza sesión desde caché al volver a local. Inicia sesión en **localhost** antes de comprar. `SESSION_SECURE_COOKIE=false` en `.env` para localhost; si ves **419**, borra cookies o usa ventana privada.
...
**Cargos operativos** (tabla `positions`, no roles Spatie): slug `despachador` (alistamiento y despacho; permiso `module.orders`) y slug `domiciliario` (entregas; permiso `module.courier`). Políticas en `App\Policies\OrderPolicy` y alcance de pedidos en `App\Support\Orders\OrderOperationsScope` (admin ve todo; despachador ve pedidos propios + pendientes sin tomar). Dashboards: `App\Policies\DashboardPolicy` + gates `viewExecutiveDashboard` / `viewDispatcherDashboard`.
...
| Dashboard ejecutivo (admin) | `/admin/dashboard`, feed JSON `/admin/dashboard/feed` — KPIs globales, ingresos, ranking despachadores |
| Panel despachador | `/despacho/dashboard`, feed `/despacho/dashboard/feed` — métricas personales sin datos financieros |
| Dashboard genérico | `/dashboard` — redirige según rol (admin → ejecutivo, despachador → panel despacho) |
| Panel admin (atajo) | `GET /admin` redirige al dashboard del rol |
| Pedidos (operaciones) | `/admin/pedidos` (admin: todos; despachador: **mis pedidos** + cola pendiente sin tomar), `/admin/pedidos/mapa`, `POST …/reasignar-despachador` (solo admin), ticket `/admin/pedidos/{order}/ticket` — permiso `module.orders`; ownership al pasar a *Preparando* (`handled_by_user_id`, `handled_at`); SLA configurable `config('orders.dispatch_sla_minutes')` (default 90) |
...
- **Carrito en sesión:** líneas `product:{id}:{kg|lb}` u `offer:{id}` (packs); `App\Services\Catalog\CartStorage` (sesión + caché compartida localhost/ngrok; vaciado en pago aprobado vía webhook y poll). Servicios: `CartSessionService`, `ProductBestPriceResolver`, etc. Badge: `resources/js/cartBadge.js` + `bfUpdateCartCount()`.
- **Cantidades de compra enteras:** `CartSessionService::normalizeQuantity()` redondea a entero (mín. 1) y `maxPurchasableUnits()` calcula el máximo comprable por unidad según el stock (kg: `floor(stock)`, lb: `floor(stock × 2)`). El componente `x-store.product-purchase` muestra «Disponibles: N kg/lb», fija `:max` y recorta la cantidad (`clampQty`, `volumeScalePricing.js`). Si se excede, el carrito responde «Solo quedan N … disponibles» o «… está agotado».
- **Sin stock = «Agotado» (no se oculta):** un producto con `stock = 0` mantiene `status = available` y **sigue listado** en el catálogo público con etiqueta **Agotado** (no comprable); `ProductStockPayload`/`ProductAvailabilityUpdated` lo reflejan en vivo. Los **combos/packs** que lo contienen se ocultan automáticamente (`OfferAvailabilityService`, `availableUnits = 0`).
- **Stock en vivo en el panel:** la tabla `/catalogo/productos` se actualiza por WebSocket (`product.stock.updated` → `bfPatchCatalogStockRow`, `stockUi.js`) sin recargar, además de Inventario y dashboard. **Respaldo por polling** (cuando el WS no conecta, p. ej. navegando por túnel HTTPS): `catalogStockPolling.js` consulta cada 15 s `GET /catalogo/productos/stock.json` (`catalog.products.stock-feed` → `ProductController@stockFeed`) y aplica el mismo parche de fila.
- **Stock mostrado con precisión:** el panel y los parches realtime muestran el stock real con hasta 2 decimales sin redondear hacia arriba (p. ej. `0.97` → `0,97 kg`, no `1,0`), para que el máximo comprable entero coincida con lo visible.
...
- **Pagos en línea:** arquitectura multi-pasarela (`PaymentGatewayInterface`, `PaymentGatewayManager`; Wompi activo). Flujo: checkout → widget Wompi → webhook → pedido + operaciones + **vacía carrito** (`PaymentWebhookProcessor` + `CartStorage::forgetForUser`). En local: `APP_URL` = túnel HTTPS; `APP_LOCAL_URL` = navegación habitual. Handoffs: `bf_tunnel_handoff`, `bf_local_handoff`. Éxito en ngrok sin redirección automática; poll `/pago/estado` en el túnel. Tests: `PaymentLocalDevelopmentFlowTest`, `CartStorageCrossHostTest`, `PaymentPollTest`, `PaymentWebhookFlowTest`.
...
- **Operaciones y despacho:** al pasar a **Preparando** el despachador que toma el pedido queda registrado en `orders.handled_by_user_id` / `handled_at` (`OrderDispatcherAssignmentService::claimForPreparing`; admin puede **reasignar** con `POST /admin/pedidos/{order}/reasignar-despachador`). El despachador ve en `/admin/pedidos` solo **mis pedidos** y la cola **pendiente sin tomar**; el admin ve todos. Al **Marcar listo para recoger** el pedido queda sin domiciliario; se notifica a **todos los domiciliarios disponibles** (`available_couriers`). En `/domiciliario/pedidos` ven la cola **Disponibles para tomar** actualizada **en vivo** (`OrderUpdated` → canal privado `couriers.pool`; tarjeta vía `GET /domiciliario/pedidos/{order}/tarjeta-pool`; polling 15 s; respaldo al recibir notificación `order_ready_for_delivery`). Botón **Aceptar** (`POST /domiciliario/pedidos/{order}/aceptar`, `CourierAssignmentService::claimByCourier` con bloqueo). Operaciones puede **asignar manualmente** en `/admin/pedidos/{order}` (`POST …/asignar-domiciliario`). Timeout configurable: `ORDER_COURIER_CLAIM_TIMEOUT_MINUTES` / `config/orders.php`; SLA despacho: `dispatch_sla_minutes` (default 90). Si nadie acepta, `php artisan orders:notify-unclaimed-ready` alerta a operaciones (scheduler cada 15 min). Grid en vivo (`order.updated`) + **mapa operativo** + GPS domiciliario como antes. Requiere **Reverb + cola** y **`npm run build`** tras cambios JS.
- **Dashboards operacionales:** panel **ejecutivo** admin (`/admin/dashboard`, feed `/admin/dashboard/feed`) con ingresos, ranking de despachadores y stock bajo — `AdminExecutiveDashboardService`, vista `admin/dashboard/executive.blade.php`, JS `executiveDashboard.js` (heatmap + polling). Panel **despachador** (`/despacho/dashboard`, feed `/despacho/dashboard/feed`) sin datos financieros — `DispatcherDashboardService`, componente compartido `x-ops.dashboard-analytics`, JS `dispatcherDashboard.js`. Analíticas compartidas: `OperationalAnalyticsService`.
...
Cobertura relevante: flujo operacional de pedidos (`OrderOperationsFlowTest`, `CourierOrderClaimTest`, `OrderDispatcherOwnershipTest`, `MarkReadyBroadcastTest`, **`CourierPoolRealtimeTest`**), configuración empresa (`CompanySettingsTest`), historial y seguimiento cliente (`CustomerOrderHistoryTest`, `OrderTrackingTimelineTest`, `CustomerTrackingMapTest`), **notificaciones** (`NotificationSystemTest`, `NotificationFeedScopeTest`, `SupplierNotificationCenterTest`), **broadcasting / Reverb** (`tests/Feature/Broadcasting/`, `tests/Feature/Realtime/`, incl. `CourierLocationBroadcastTest`), pagos Wompi (`tests/Feature/Payments/`), carrito, catálogo público compacto y escalas por volumen (`tests/Feature/Store/`, incl. `PublicCatalogViewsTest`), catálogo admin de ofertas (`tests/Feature/Catalog/`), redirects post-login (`PostLoginRedirectTest`).
...
|-------------|-------------|
| Despachador | `despachador1@demo.beeffresh.test`, `despachador2@demo.beeffresh.test` |
